from processing import vision_processing
from processing import bounding_box_projection
...
if __name__ == "__main__":
    ...
    boxes_dct = predictor.get_boxes(model, vid1_path, toi1)
    boxes_dct2 = predictor.get_boxes(model, vid2_path, toi2)
    # Converts the boxes to fittable format and writes to dataframe
    df = predictor.convert_boxes_df(boxes_dct)
    df2 = predictor.convert_boxes_df(boxes_dct2)
    # Adds the center of the boxes to the dataframe
    df = bounding_box_projection.add_center(df)
    df2 = bounding_box_projection.add_center(df2)
    # Reads video data
    vid_data = bounding_box_projection.read_video_data(vid1_path)
    vid2_data = bounding_box_projection.read_video_data(vid2_path)
    # shrinks the dataframe to the time of interest
    df = df[(df['frame'] >= toi1[0]) & (df['frame'] <= toi1[1])]
    df = bounding_box_projection.eliminate_outliers(df)
    df = bounding_box_projection.normalize_boxes(df, toi1)
    df.to_csv(boxes_path)
    df2 = df2[(df2['frame'] >= toi2[0]) & (df2['frame'] <= toi2[1])]
    df2 = bounding_box_projection.eliminate_outliers(df2)
    df2 = bounding_box_projection.normalize_boxes(df2, toi2)
    df2.to_csv(boxes2_path)
    vision_processing.get_circles(df, vid1_path, out1_path)
